...
<#
    ...
    - simulation: this mock version fakes all changes for UI preview
...
# Simulation mode for macOS / dry-run preview. No real writes.
$Simulate = $true
...
$Script:SimServices = @(
    ...
    [pscustomobject]@{ Name = "dmwappushservice"; Status = "Stopped" },
    [pscustomobject]@{ Name = "CopilotService"; Status = "Running" }
)
$Script:SimTasks = @(
    [pscustomobject]@{ TaskPath = "\Microsoft\Windows\Application Experience\"; TaskName = "Microsoft Compatibility Appraiser"; State = "Ready" },
    [pscustomobject]@{ TaskPath = "\Microsoft\Windows\Customer Experience Improvement Program\"; TaskName = "Consolidator"; State = "Ready" },
    [pscustomobject]@{ TaskPath = "\Microsoft\Windows\WindowsCopilot\"; TaskName = "CopilotTask"; State = "Ready" }
)
...
function Get-RegistryValue {
    ...
    if ($Simulate) {
        return $Script:SimReg["$($Target.Path)|$($Target.ValueName)"]
    }
...
function Set-RegistryValue {
    ...
    if ($Simulate) {
        $Script:SimReg["$($Target.Path)|$($Target.ValueName)"] = $Value
        Write-Host ("   (simulated write) {0}\{1} = {2}" -f $Target.Path, $Target.ValueName, $Value) -ForegroundColor $Colors.Muted
        return
    }
...
function Get-CopilotServices {
    if ($Simulate) {
        return $Script:SimServices | Where-Object { $_.Name -match "copilot" }
    }
...
function Get-CopilotTasks {
    if ($Simulate) {
        return $Script:SimTasks | Where-Object { $_.TaskName -match "copilot" -or $_.TaskPath -match "copilot" }
    }
...
function Show-QuickHelp {
    Write-Host "`n[What happens]" -ForegroundColor $Colors.Title
...
function Disable-TelemetryServicesAndTasks {
    ...
    foreach ($svcName in $Script:TelemetryServices) {
        try {
            if ($Simulate) {
                $match = $Script:SimServices | Where-Object { $_.Name -ieq $svcName }
            ...
            Write-Host (" - Service {0}: stopped + disabled" -f $svcName) -ForegroundColor $Colors.Success
        } catch {
    ...
    foreach ($taskPath in $Script:TelemetryTasks) {
        try {
            ...
            if ($Simulate) {
                $match = $Script:SimTasks | Where-Object { $_.TaskPath -eq $taskFolder -and $_.TaskName -eq $taskName }
                if ($match) { $match.State = "Disabled" }
            } else {
